...
# "405451727016-5udopg51n6v76ro82cd05f66dsq7hl90.apps.googleusercontent.com"
...
# "mongodb+srv://Kadambari:Kadambari@cluster0.zjfwjys.mongodb.net/resumescreener?retryWrites=true&w=majority"
mongo.init_app(app)
...
def hr():
    ...
    #if 'user_id' in session and session['user_id'] == 'hr':
        return render_template("HRDashboard.html")
    else:
        return render_template("index.html", errMsg="Invalid Login") #login first
...
@app.route('/HRlog', methods=['GET', 'POST'])
def index1():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']

        # Check if HR credentials are correct
        if username == 'hr' and password == 'hr123':
            # Add HR details to the session
            session['user_id'] = 'hr'
            session['user_name'] = 'HR Manager'
            return render_template('CompanyDashboard.html') #index.html
        else:
            # Invalid HR credentials, show error message on login page
            return render_template('index.html', hrErrMsg='Invalid HR credentials')

    # If GET request or invalid HR credentials, show login page
    return render_template('index.html')
